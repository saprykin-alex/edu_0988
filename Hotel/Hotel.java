...
public class Hotel {
    ...
    public void getFreeRooms(String[] prop){
        ...
        //Передаём на обработку список комнат и параметры
    ...
    public void getRoomsWith(String[] prop){
        //Создаём полный список всех комнат
        ArrayList<Room> allRooms = new ArrayList<>();
        for (int i = 0; i < rooms.length; i++)
            allRooms.add(rooms[i]);
        //Передаём на обработку полный список комнат и параметры
    ...
    private void getRooms(ArrayList<Room> selectedRooms, String[] prop){
        ...
        boolean[] roomServices = {false, false, false, false};        //wc, eat, wifi, conditioner
        boolean isRoomServices = false;
        boolean[] roomQuantity = {false, false, false, false};        //1, 2, 3, 4
        ...
        for (int i = 0; i < prop.length; i++) {
            ...
            if (prop[i].equals("conditioner") || prop[i].equals("Conditioner") || prop[i].equals("condition") || prop[i].equals("Condition") || prop[i].equals("cond")){
                roomServices[3]=true;
                isRoomServices = true;
            }
            ...
            if (prop[i].equals("1") || prop[i].equals("sleepingPlace:1")){
                roomQuantity[0]=true;
                isRoomQuantity = true;
            }
            if (prop[i].equals("2") || prop[i].equals("sleepingPlace:2")){
                roomQuantity[1]=true;
                isRoomQuantity = true;
            }
            if (prop[i].equals("3") || prop[i].equals("sleepingPlace:3")){
                roomQuantity[2]=true;
                isRoomQuantity = true;
            }
            if (prop[i].equals("4") || prop[i].equals("sleepingPlace:4")){
                roomQuantity[3]=true;
                isRoomQuantity = true;
            }
            ...
            for (int i = 0; i < 4; i++) {
                if (roomQuantity[i]) {
                    for (int j = 0; j < selectedRooms.size(); j++) {
                        ...
                        if (i == 3) {
                            if (selectedRooms.get(j).getQuantity() == 4) {
                                itogList.add(selectedRooms.get(j));
                            }
                        }
                    }
        ...
        for (int i = 0; i < 4; i++) {
            if (roomServices[i]){
                for (int j = 0; j < selectedRooms.size(); j++) {
                    ...
                    if (i==3){
                        if (selectedRooms.get(j).isConditioner()) itogList.add(selectedRooms.get(j));
                    }
                }
            }else continue;
            selectedRooms.clear();
    ...
    public void propertiesAllRooms(){
        ...
        System.out.println(" Номер \t\tКоличество\tНаличие\t\tПитание\t\tНаличие\t\t  Наличие\t\tНомер");
        System.out.println("комнаты\t\t   мест   \tтуалета\t\tвключено\t Wi-Fi \t\tкондиционера\tзанят");
        ...
        for (int i = 0; i < rooms.length; i++) {
            ...
            if (rooms[i].isConditioner()) System.out.print("Да\t\t\t "); else System.out.print("Нет\t\t ");
